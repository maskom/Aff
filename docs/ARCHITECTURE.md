...
- Validate all environment variables in Nuxt runtime config before bootstrapping using zod schema validation (implemented in AFF-04).
- Environment variables validated at build time: `NUXT_PUBLIC_APP_NAME`, `NUXT_PUBLIC_API_BASE`, `NUXT_ENVIRONMENT`.
- Worker validates its environment on each request: `AFF_STATIC` (required), `AFF_API_BASE` (optional URL), `ENVIRONMENT`.
