...
- Use `pnpm lint:fix` to auto-fix linting issues where possible.
- Format code with Prettier (`pnpm format`). Check formatting with `pnpm format:check`.
- Run `pnpm typecheck` to verify TypeScript types.
- Do not disable lint rules without team agreement.
