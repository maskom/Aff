...
| AFF-01 | Set up GitHub Actions CI         | Phase 1 | DevOps           | 3d       | Done   | `.github/workflows/ci.yml` runs lint/typecheck/format/build on PR; status checks required on `main`.   |
| AFF-02 | Modularize landing components    | Phase 1 | Frontend         | 5d       | Done   | `app.vue` refactored: Header, BottomNav, StoriesSection, OfferList under `components/affiliate/`.      |
...
| AFF-04 | Runtime config schema            | Phase 1 | Backend          | 1d       | Done   | `nuxt.config.ts` validates env vars using `zod`; build fails on invalid config in production.          |
