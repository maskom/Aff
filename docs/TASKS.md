...
| AFF-01 | Set up GitHub Actions CI         | Phase 1 | DevOps           | 3d       | Done   | Workflow runs lint/test/build on PR; status checks required on `main`.                                 |
| AFF-02 | Modularize landing components    | Phase 1 | Frontend         | 5d       | Done   | `app.vue` refactored into components under `components/affiliate/` with unit tests.                    |
| AFF-03 | Configure linting toolchain      | Phase 1 | Tooling          | 2d       | Done   | ESLint, Prettier scripts added; Vue/TS linting configured. Husky hooks configured.                     |
...
| AFF-05 | Cloudflare deployment pipeline   | Phase 1 | DevOps           | 4d       | Done   | Wrangler GitHub Action deploys to staging; manual approval promotes to production.                     |
