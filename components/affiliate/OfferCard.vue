<template>
  <li class="offer-card" :data-testid="`offer-${offer.id}`">
    ...
    <div class="offer-card__body">
      <div class="offer-card__header">
        <div>
          <p class="offer-card__brand">{{ offer.brand }}</p>
          <p class="offer-card__product">{{ offer.product }}</p>
        </div>
        <span class="offer-card__earnings">{{ offer.commission }}</span>
      </div>
      <p class="offer-card__description">{{ offer.description }}</p>
      <div class="offer-card__meta">
        <span class="offer-card__tag">{{ offer.category }}</span>
        <span class="offer-card__meta-separator">•</span>
        <span class="offer-card__meta-text">Update {{ offer.lastUpdate }}</span>
        <span class="offer-card__meta-separator">•</span>
        <span class="offer-card__meta-text">Konversi {{ offer.conversion }}%</span>
      </div>
    </div>
    <button class="offer-card__cta" type="button" :data-testid="`cta-${offer.id}`">
      Promosikan
    </button>
  </li>
</template>
...
.offer-card__meta-separator {
  color: rgba(231, 241, 237, 0.3);
}

.offer-card__meta-text {
  white-space: nowrap;
}
